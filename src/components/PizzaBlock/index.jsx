...
function PizzaBlock({ pizza }) {
  const { id, imageUrl, name, price, sizes, types, count } = pizza;
  const [activeType, setActiveType] = React.useState(0);
  const [activeSize, setActiveSize] = React.useState(0);

  const dispatch = useDispatch();
  const addedItem = useSelector(cartItemSelectorById(id));

  const hendlerClick = (index, setActive) => {
    setActive(index);
  };
  const onClickAdd = () => {
    const addedItem = {
            ...
            {types.map((typeIndex, index) => {
              return (
                <li
                  ...
                  onClick={() => hendlerClick(typeIndex, setActiveType)}>
                  {typeNames[typeIndex]}
                </li>
              );
            })}
            ...
            {sizes.map((size, index) => {
              return (
                <li
                  ...
                  onClick={() => hendlerClick(index, setActiveSize)}>
                  {size} см.
                </li>
              );
            })}
